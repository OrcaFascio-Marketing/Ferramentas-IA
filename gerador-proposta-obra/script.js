...
const elistData = {
  'tipo-obra':[],'regime':[],'pagamento':[],'reajuste':[],'premissas':[],'servicos-custom':[],
};
...
const EXCL_SUGESTOES = [
  "Mobiliário e decoração","Paisagismo e jardinagem avançada","Equipamentos de cozinha",
  "Sistema de automação","Painéis fotovoltaicos","Piscina e spa","Elevadores e plataformas",
  "Climatização central","Projetos executivos","Análise de solo / sondagem",
...
// ─── TAGS EXCLUSÕES ──────────────────────────────────────────────────────────
function addExcl(t){if(exclTags.includes(t)){showToast('Item já adicionado.');return;}exclTags.push(t);renderExcl();}
function removeExcl(t){exclTags=exclTags.filter(x=>x!==t);renderExcl();}
function renderExcl(){
...
function validatePage(n){
  ...
  if(n===2){
    ...
    if(!checked.length&&!elistData['servicos-custom'].length){showToast('Selecione ou adicione ao menos um serviço.');return false;}
    if(!document.getElementById('prazo-num').value.trim()){showToast('Informe o prazo de execução.');return false;}
    if(!elistData['pagamento'].length){showToast('Adicione ao menos uma forma de pagamento/medição.');return false;}
  }
  return true;
}
function voltarForm(){
...
function gerarPrompt(){
  ...
  promptGerado=
`Você é um especialista em vendas e elaboração de propostas comerciais para o setor da construção civil no Brasil.

Crie uma PROPOSTA COMERCIAL DE SERVIÇOS profissional, objetiva e persuasiva. O tom deve ser claro e direto — de empresa para cliente, não de advogado para juiz. O objetivo é convencer o cliente e facilitar a decisão de contratar.

DADOS DA OBRA:
- Tipo de obra: ${d.tipoObra}
- Regime de execução: ${d.regime}
- Local: ${d.local}${d.area?'\n- Área: '+d.area:''}${d.valor?'\n- Valor estimado: '+d.valor:''}
- Descrição: ${d.objeto||(d.tipoObra?'(elabore com base no tipo de obra)':'não informado')}
- Prazo de execução: ${d.prazo||'a definir'}
- Início previsto: ${d.dataInicio}
- Forma de pagamento: ${d.pagamento}
- Reajuste: ${d.reajuste}

SERVIÇOS INCLUÍDOS:
${d.servicos.length?d.servicos.map(s=>'- '+s).join('\n'):'- A detalhar conforme projeto'}
...
- Texto corrido, SEM Markdown (sem #, ##, **, *, _ ou qualquer marcação)
- Títulos de seção: somente o número e o nome, em linha própria. Ex: "1. Sobre esta Proposta"
- Listas: hífen simples no início. Ex: "- item"
- Tabelas: formato "Campo | Valor", um por linha
- NÃO use hashtags, asteriscos, underlines nem colchetes de link

Para cada seção, inclua UMA linha de orientação entre [[colchetes duplos]] com dica prática para o profissional personalizar antes de enviar. Seja conciso nessas orientações.

Gere as seguintes seções com linguagem comercial, direta e profissional:

1. Sobre esta Proposta
[[Uma ou duas frases apresentando a proposta e a empresa. Personalize com o nome da construtora e um diferencial relevante para este tipo de obra.]]
Texto de apresentação da proposta: o que está sendo oferecido, por quem, e por quê o cliente pode confiar.

2. Escopo dos Serviços
[[Liste os serviços de forma clara. Use marcadores. Destaque o que está INCLUÍDO e o que NÃO está incluído, separados.]]
O que está incluído nesta proposta e o que não está (exclusões objetivas, sem juridiquês).

3. Como trabalhamos
[[Descreva o método de trabalho: equipe, processos, qualidade, comunicação com o cliente. 3 a 5 pontos que diferenciam a empresa.]]
Breve descrição do processo de execução, diferenciais da empresa e como o cliente será atendido durante a obra.

4. Investimento e Condições Comerciais
[[Preencha os dados abaixo com os valores reais desta proposta. Use o formato "Rótulo | Valor" para cada item — isso gera um visual de tabela na ferramenta.]]
Valor da proposta | (preencher)
Prazo de execução | ${d.prazo||'a definir'}
Início previsto | ${d.dataInicio}
Forma de pagamento | ${d.pagamento}
Reajuste | ${d.reajuste}
Validade desta proposta | ${d.validadeNum ? d.validadeNum+' '+d.validadeUnidade : '30 dias'}

5. Próximos passos
[[Sugira 2 ou 3 ações concretas para o cliente avançar: reunião, visita técnica, assinatura. Tom propositivo.]]
O que o cliente deve fazer para avançar com a contratação. Tom encorajador e proativo.`
;
...
function formatarEscopo(){
  ...
  const logosHtmlBloco = temLogos ? `<div class="doc-logos">
    ${blocoLogoContratado}
  ...
  const attHtml = (d.repContratante||d.nomeContratante) ? `<div class="doc-att">
  ...
  function rdLinha(label, valor){ return valor ? `<tr><td>${label}</td><td>${esc(valor)}</td></tr>` : ''; }
  ...
  <div class="doc-data-validade">
    ...
    ${d.validadeNum ? `<div class="doc-data-item">
  ...
  const rodapeHtmlFinal = rodapeTexto
    ? `<div class="rodape-doc">${rodapeTexto}</div>`
    : '';
...
const TC_SIGLAS = new Set([
  'abnt','art','arts','bim','bndes','cef','cnpj','cpf','cnd','crea','cau',
  'art','rrt','spda','incc','ipca','cub','fgv','itbi','iss','icms','pis',
  'cofins','gf','nbr','iso','cdc','iss','inss','fgts','a4','s/a','ltda',
  'mei','epp','eire','sa'
]);
...
function md2html(md){
  ...
  // 5b. "Rescisão Contratual" — seção inadequada em proposta comercial
  // Apenas filtrar se vier com linguagem jurídica (detectamos pela seção toda)
  // NÃO filtrar aqui — deixar o usuário decidir via orientação [[...]]
  ...
  md=md.replace(/\n{3,}/g,'\n\n');
  // 9b. Remover linha literal "Campo | Valor" (cabeçalho de tabela que a IA gera)
  md=md.replace(/^\s*Campo\s*\|\s*Valor\s*$/gim,'');
  ...
  let lines=md.split('\n');
  let out=[],inList=false,listType='',inTableHeader=false;
  ...
  for(let raw of lines){
    ...
    // Numbered list vs. título de seção numerado
    if(/^\d+\.\s+(.+)$/.test(line)){
      const nm = line.match(/^(\d+)\.\s+(.+)$/);
      const num = parseInt(nm[1]);
      const txt = nm[2];
      // É título de seção: número 1-20, ≥2 palavras, começa maiúscula, NÃO estamos em lista
      const isSec = num>=1 && num<=20 && txt.split(/\s+/).length>=2
                 && /^[A-ZÁÉÍÓÚÂÊÔÀÃÕ]/.test(txt) && !inList;
      if(isSec){
        closeList();
        out.push(`<h2>${inlineFormat(titleCase(num+'. '+txt))}</h2>`);
        continue;
      }
      if(!inList||listType!=='ol'){closeList();out.push('<ol>');inList=true;listType='ol';}
      out.push(`<li>${inlineFormat(txt)}</li>`); continue;
    }
    // Table rows — detecta tanto "| A | B |" quanto "A | B" (sem bordas)
    const hasPipe = line.includes('|');
    const isPipedRow = /^\|.+\|$/.test(line);          // | A | B |
    const isInlineRow = !isPipedRow && /^[^|]+\|[^|]+$/.test(line.trim()); // A | B
    if(isPipedRow || (hasPipe && isInlineRow)){
      closeList();
      if(/^[\s|\-]+$/.test(line)) continue; // separator row (--- | ---)
      let cells;
      if(isPipedRow){
        cells = line.split('|').slice(1,-1).map(c=>c.trim()).filter(Boolean);
      } else {
        cells = line.split('|').map(c=>c.trim()).filter(Boolean);
      }
      if(!cells.length) continue;
      // Primeira linha de tabela abre <table>
      const lastOut = out[out.length-1]||'';
      if(!lastOut.startsWith('<tr>') && !lastOut.startsWith('</tr>')){
        if(inTableHeader){
    ...
    // Fechar tabela quando linha não é pipe
    {
      const lastOut = out[out.length-1]||'';
      if(lastOut.startsWith('<tr>') || lastOut === '</tr>'){
        out.push('</table>');
      }
    }
    // Empty line
  ...
  // Wrap tables
  let html=out.join('\n');
  // Envolver linhas <tr> que não têm <table> ao redor
  ...
  // Remover duplo </table><table> (tabelas fragmentadas que deveriam ser uma só)
...
<!--[if gte mso 9]><xml><w:WordDocument><w:View>Print</w:View><w:Zoom>100</w:Zoom><w:DoNotOptimizeForBrowser/></w:WordDocument></xml><![endif]-->
  ...
  a.href=url; a.download='Escopo-Proposta-Obra.doc';
  document.body.appendChild(a); a.click();
  document.body.removeChild(a); URL.revokeObjectURL(url);
  btn.disabled=false; btn.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg> Word (.docx)';
...
function baixarPptx(){
  ...
  // Extrair seções do documento formatado
  const sections=[];
  let current=null;
  docEl.querySelectorAll('h1,h2,h3,p,ul,ol,table').forEach(el=>{
    if(el.classList.contains('orientacao')) return;
    if(el.tagName==='H2'){
      if(current) sections.push(current);
      current={title:el.innerText.trim(), items:[]};
    } else if(current){
      if(el.tagName==='P' && el.innerText.trim()){
        current.items.push({type:'p', text:el.innerText.trim()});
      } else if(el.tagName==='UL'||el.tagName==='OL'){
        [...el.querySelectorAll('li')].forEach(li=>{
          current.items.push({type:'li', text:li.innerText.trim()});
        });
      } else if(el.tagName==='TABLE'){
        [...el.querySelectorAll('tr')].forEach(tr=>{
          const cells=[...tr.querySelectorAll('th,td')].map(c=>c.innerText.trim());
          if(cells.length) current.items.push({type:'tr', cells});
        });
      }
    }
  });
  ...
  // Gerar HTML com layout de slides — importável no Google Slides / Canva
  const W=960, H=540;
  const pri=cores.primaria, sec=cores.secundaria;

  let html=`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:Arial,sans-serif;background:#f0f0f0}
.slide{width:${W}px;min-height:${H}px;background:#fff;margin:20px auto;
  box-shadow:0 4px 20px rgba(0,0,0,.15);position:relative;overflow:hidden;
  page-break-after:always;display:flex;flex-direction:column}
.slide-capa{background:${sec};color:#fff;justify-content:center;align-items:center;text-align:center;padding:60px 80px}
.slide-capa h1{font-size:36px;font-weight:800;margin-bottom:12px;line-height:1.2}
.slide-capa h2{font-size:18px;font-weight:400;opacity:.75}
.slide-capa .data{font-size:13px;opacity:.5;margin-top:20px}
.slide-content{padding:40px 56px 36px}
.slide-header{border-bottom:3px solid ${pri};padding-bottom:10px;margin-bottom:22px}
.slide-header h2{font-size:20px;font-weight:700;color:${sec}}
.slide-body{flex:1}
.slide-body p{font-size:14px;line-height:1.6;color:#333;margin-bottom:10px}
.slide-body li{font-size:13px;line-height:1.55;color:#333;margin-bottom:5px;
  padding-left:14px;position:relative}
.slide-body li::before{content:"•";position:absolute;left:0;color:${pri};font-weight:700}
.slide-body table{width:100%;border-collapse:collapse;font-size:12px;margin-top:6px}
.slide-body td:first-child{font-weight:600;color:#555;width:40%;padding:5px 10px;
  background:#f8f8f8;border:1px solid #e0e0e0}
.slide-body td:last-child{padding:5px 10px;border:1px solid #e0e0e0}
.slide-num{position:absolute;bottom:14px;right:20px;font-size:11px;color:#bbb}
.slide-accent{position:absolute;bottom:0;left:0;width:100%;height:4px;background:${pri}}
@media print{
  body{background:#fff}
  ...
  // Slide de capa
  html+=`<div class="slide slide-capa">
    <h1>Proposta de Serviços</h1>
    ${d.tipoObra?`<h2>${d.tipoObra}</h2>`:''}
    ${d.nomeContratado?`<div class="data" style="margin-top:16px;font-size:15px;opacity:.8">${d.nomeContratado}</div>`:''}
    <div class="data">${d.dataDocumento}</div>
  </div>`;

  // Um slide por seção
  sections.forEach((sec,i)=>{
    const MAX=10; // máximo de itens por slide
    const items=sec.items.slice(0,MAX);
    const rest=sec.items.slice(MAX);
    function slideHTML(title, its, num){
      let body='';
      // Agrupar: parágrafos em texto corrido, listas em <ul>, tabela
      let trs=its.filter(x=>x.type==='tr');
      let paras=its.filter(x=>x.type==='p');
      let lis=its.filter(x=>x.type==='li');
      if(paras.length) body+=paras.slice(0,3).map(x=>`<p>${x.text.slice(0,220)}</p>`).join('');
      if(trs.length){
        body+='<table>';
        trs.slice(0,8).forEach(r=>{ body+=`<tr>${r.cells.map(c=>`<td>${c}</td>`).join('')}</tr>`; });
        body+='</table>';
      }
      if(lis.length) body+=lis.slice(0,8).map(x=>`<li>${x.text.slice(0,160)}</li>`).join('');
      return `<div class="slide"><div class="slide-content">
        <div class="slide-header"><h2>${title}</h2></div>
        <div class="slide-body">${body}</div>
        <div class="slide-num">${num}</div>
      </div><div class="slide-accent"></div></div>`;
    }
    html+=slideHTML(sec.title, items, i+2);
    if(rest.length) html+=slideHTML(sec.title+' (cont.)', rest, i+3);
  });
  ...
  const a=document.createElement('a');
  a.href=url;
  const nome=(d.nomeContratado||'proposta').replace(/[^a-zA-Z0-9]/g,'-').toLowerCase().slice(0,30);
  a.download=`slides-${nome}.html`;
  document.body.appendChild(a); a.click();
  document.body.removeChild(a); URL.revokeObjectURL(url);

  btn.disabled=false;
  btn.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg> Apresentação (.html)';
  showToast('✓ Slides gerados! Abra o arquivo e imprima para PDF, ou importe no Google Slides.');
}
...
function copiarAbrirIA(){
  ...
  function abrirIA(){
    // Para todos: abrir nova aba + avisar para colar
    // (URL com ?q= é bloqueada por browsers para prompts longos)
    window.open(URLS[selectedAI], '_blank');
    showToast('✓ Prompt copiado! Cole com Ctrl+V no ' + NOMES[selectedAI] + '.');
  }

  // Tentar clipboard moderno, fallback para execCommand
  if(navigator.clipboard && navigator.clipboard.writeText){
    navigator.clipboard.writeText(promptGerado)
      .then(abrirIA)
      .catch(()=>{ copiarFallback(); abrirIA(); });
  } else {
    copiarFallback(); abrirIA();
  }
}
...
function salvarHTML(){
  ...
  const nome = (state.fields['nome-contratado']||'proposta').replace(/[^a-zA-Z0-9çãõáéíóúâêôàü ]/gi,'').trim().replace(/\s+/g,'-').toLowerCase().slice(0,40);
...
function importarHTML(input){
  const file = input.files[0]; if(!file) return;
  if(!file.name.endsWith('.html')){showToast('Por favor, importe um arquivo .html gerado por esta ferramenta.');return;}
  const reader = new FileReader();
  reader.onload = e => {
    try {
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  document.getElementById('excl-sugestoes').innerHTML=EXCL_SUGESTOES.map(s=>
    `<span class="chip" onclick="addExcl('${esc(s)}')">${s}</span>`
  ).join('');
